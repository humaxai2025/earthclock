  ...
  >
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-center mt-2">
        <div
          className={`text-xs opacity-60 text-center ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}
        >
          <div>Last updated: {new Date(environmentalData.lastUpdate).toLocaleTimeString()}</div>
          {environmentalData.error && (
            <div className="text-yellow-500 mt-1">⚠️ {environmentalData.error}</div>
          )}
          {environmentalData.sources && environmentalData.isRealTime && (
            <div className="mt-1 text-xs opacity-50">
              Sources:{' '}
              {Object.values(environmentalData.sources).filter(Boolean).slice(0, 2).join(', ')}
            </div>
          )}
        </div>
      </div>
