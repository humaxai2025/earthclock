  ...
  return (
    ...
    >
      <h4 className="font-semibold mb-2 flex items-center">
        <span
          className={`${animationClass} text-lg mr-2`}
          style={animationClass === 'animate-spin' ? { animationDuration: '3s' } : {}}
        >
          {emoji}
        </span>
        {title}
      ...
      {value && (
        <div
          className={`text-2xl font-bold ${styles.valueText} mb-1 group-hover:scale-110 transition-transform duration-300`}
        >
          {value}
        </div>
      )}
      {subtitle && <div className="text-sm opacity-80 mb-2">{subtitle}</div>}
      {width && (
        <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
          <div
            className={`${styles.progressBar} h-2 rounded-full transition-all duration-1000 relative`}
