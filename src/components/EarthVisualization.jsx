...
const EarthVisualization = ({ isDarkMode }) => (
  <div className={`p-6 rounded-3xl backdrop-blur-xl shadow-2xl border transition-all duration-700 hover:scale-[1.02] group flex-1 ${
    ...
    <div className="flex flex-col items-center justify-center h-full">
      ...
      <div className="relative w-40 h-40 group-hover:scale-110 transition-transform duration-500">
        ...
        {/* Outer Space Glow */}
        <div className="absolute inset-0 rounded-full bg-gradient-radial from-blue-400/20 via-cyan-300/10 to-transparent animate-pulse" 
             style={{transform: 'scale(1.8)', animationDuration: '4s'}} />
        
        {/* Aurora-like Atmospheric Effects */}
        <div className="absolute inset-0 rounded-full aurora-ring-1" />
        <div className="absolute inset-1 rounded-full aurora-ring-2" />
        <div className="absolute inset-2 rounded-full aurora-ring-3" />
        
        {/* Subtle Orbital Ring */}
        <div className="absolute inset-0 rounded-full border border-emerald-400/20 simple-pulse-ring" />
        
        {/* Main Earth Sphere with Enhanced Gradients */}
        <div className={`absolute inset-6 rounded-full soft-earth-pulse earth-shadow ${
          isDarkMode 
            ? 'bg-gradient-to-br from-blue-400 via-green-500 to-cyan-600' 
            : 'bg-gradient-to-br from-emerald-400 via-blue-500 to-teal-600'
        }`} style={{
          ...
          {[...Array(8)].map((_, i) => (
            <div
              ...
              className={`absolute w-1 h-1 rounded-full ${
                isDarkMode ? 'bg-cyan-300/60' : 'bg-emerald-400/60'
              } floating-particle`}
              style={{
    ...
    <style>{`
      .aurora-ring-1 {
        background: conic-gradient(from 0deg, transparent, rgba(34, 197, 94, 0.3), transparent, rgba(59, 130, 246, 0.3), transparent);
        animation: spin 20s linear infinite;
      }
      
      .aurora-ring-2 {
        background: conic-gradient(from 45deg, transparent, rgba(168, 85, 247, 0.2), transparent, rgba(236, 72, 153, 0.2), transparent);
        animation: spin 25s linear infinite reverse;
      }
      
      .aurora-ring-3 {
        background: conic-gradient(from 90deg, transparent, rgba(34, 211, 238, 0.25), transparent);
