...
const EarthVisualization = ({ isDarkMode, urgencyLevel = 'critical' }) => {
  ...
  const getEarthColors = (urgency, isDark) => {
    switch (urgency) {
      case 'safe':
        return {
          outerGlow: isDark
            ? 'from-blue-400/20 via-cyan-300/10'
            : 'from-emerald-400/20 via-blue-300/10',
      ...
      case 'warning':
        return {
          outerGlow: isDark
            ? 'from-yellow-400/20 via-orange-300/10'
            : 'from-yellow-400/20 via-orange-300/10',
      ...
      default:
        return {
          outerGlow: isDark
            ? 'from-red-400/20 via-orange-300/10'
            : 'from-red-400/20 via-orange-300/10',
  ...
  return (
    <div
      className={`p-6 rounded-3xl backdrop-blur-xl shadow-2xl border transition-all duration-700 hover:scale-[1.02] group flex-1 ${
        isDarkMode
    ...
    >
      <div className="flex flex-col items-center justify-center h-full">
        ...
        <div className="relative w-40 h-40 group-hover:scale-110 transition-transform duration-500">
          {/* Outer Space Glow */}
          <div
            className={`absolute inset-0 rounded-full bg-gradient-radial ${colors.outerGlow} to-transparent animate-pulse`}
            style={{ transform: 'scale(1.8)', animationDuration: '4s' }}
          />

          {/* Aurora-like Atmospheric Effects */}
          <div
            className="absolute inset-0 rounded-full aurora-ring-1"
            style={{
              background: `conic-gradient(from 0deg, transparent, ${colors.auroraRing1}, transparent)`,
            }}
          />
          <div
            className="absolute inset-1 rounded-full aurora-ring-2"
            style={{
              background: `conic-gradient(from 45deg, transparent, ${colors.auroraRing2}, transparent)`,
            }}
          />
          <div
            className="absolute inset-2 rounded-full aurora-ring-3"
            style={{
              background: `conic-gradient(from 90deg, transparent, ${colors.auroraRing3}, transparent)`,
            }}
          />

          {/* Subtle Orbital Ring */}
          <div
            className={`absolute inset-0 rounded-full border ${colors.orbitalRing} simple-pulse-ring`}
          />

          {/* Main Earth Sphere with Enhanced Gradients */}
          <div
            className={`absolute inset-6 rounded-full soft-earth-pulse earth-shadow`}
            style={{
